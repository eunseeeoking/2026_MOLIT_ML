...
async function analyzeTradeDistribution() {
  ...
  try {
    ...
    for (let i = 0; i < Math.min(10, allComplexes.length); i++) {
      ...
      console.log(`  ${i + 1}. ${String(c.name).padEnd(30)} ${String(cnt).padStart(3)}건 ${trainable}`);
    }
    ...
    if (trainableCount > 0) {
      const sampleComplex = allComplexes.find((c) => Number(c.cnt) >= requiredTrades);
      if (sampleComplex) {
        console.log(`\n🔍 학습 가능 단지 샘플 분석 (id=${sampleComplex.id})`);

        const trades = await prisma.aptTrade.findMany({
          where: { complexId: sampleComplex.id },
        ...
        console.log(`  단지: ${sampleComplex.name}`);
        ...
        trades.forEach((t) => {
          const month = t.dealDate.toISOString().substring(0, 7);
